...
with open('countrys.json', 'r', encoding='utf-8') as f:
    Country = json.load(f)
# print(Country['United States of America']['options'])
    ...
    def getUrls(self):
        ...
        if not res:
            ...
            for row in range(1, sheet.nrows):
                ...
                self.data.append((url, status, country))
            # 全局 队列 q
...
class ProInfoSpider(Thread):
    ...
    def run2(self, country, typename, ranking):
        ...
        esselect = Country[country]['id']
        categoryid = Country[country]['options']
        print(esselect,categoryid)
        # print(country,esselect,categoryid)
        # 爬取 销量预估
        # try:
        response = requests.get(
            f'https://api.junglescout.com/api/v1/sales_estimator?rank={ranking}&category={categoryid}&store={esselect}',
            headers={
